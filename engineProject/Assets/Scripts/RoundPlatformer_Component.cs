...
public class RoundPlatformer_Component : MonoBehaviour {
	...
	void Update()
	{
		Linerender.material.mainTextureOffset += new Vector2(20*Time.deltaTime, 0);
	...
	public void RespectTextureRatio(){
	 ...
	 public void CreatePoints(float InputAngle, float Rotation){
		...
		for (int i = 0; i < (Segments + 1); i++){
			X = Mathf.Sin (Mathf.Deg2Rad * angle) * Position + transform.position.x;
			Y = Mathf.Cos (Mathf.Deg2Rad * angle) * Position + transform.position.y;
